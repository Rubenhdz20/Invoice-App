...
describe('ItemsSection', () => {
  test('shows formatted total and updates when qty/price change', async () => {
    ...
    expect(document.querySelector('input[readonly]')!).toHaveValue('$200.00');

    // ACT: change Qty from 2 → 3
    const [qtyInput, priceInput] = screen.getAllByRole('spinbutton') as HTMLInputElement[];
    await user.clear(qtyInput);
    await user.type(qtyInput, '3');

    // ASSERT: total updates → $300.00
    expect(document.querySelector('input[readonly]')!).toHaveValue('$300.00');
    ...
    expect(document.querySelector('input[readonly]')!).toHaveValue('$59.97');
  });
});